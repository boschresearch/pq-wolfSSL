...
int32_t montgomery_reduce64(int64_t a) {
  ...
  t = (int32_t)a*DILITH_QINV;
  t = (a - (int64_t)t*DILITH_Q) >> 32;
  return t;
}
...
static const int32_t dilithium_zetas[DILITH_N] = {
...
* Name:        dilithium_ntt
*
* Description: Forward dilithium_ntt, in-place. No modular reduction is performed after
...
void dilithium_ntt(int32_t a[DILITH_N]) {
  ...
  for(len = 128; len > 0; len >>= 1) {
    for(start = 0; start < DILITH_N; start = j + len) {
      zeta = dilithium_zetas[++k];
      for(j = start; j < start + len; ++j) {
        t = montgomery_reduce64((int64_t)zeta * a[j + len]);
        a[j + len] = a[j] - t;
        a[j] = a[j] + t;
      }
    }
...
/*************************************************
* Name:        dilithium_invntt_tomont
*
* Description: Inverse dilithium_ntt and multiplication by Montgomery factor 2^32.
  ...
  for(len = 1; len < DILITH_N; len <<= 1) {
    for(start = 0; start < DILITH_N; start = j + len) {
      ...
      for(j = start; j < start + len; ++j) {
        ...
        a[j] = t + a[j + len];
        a[j + len] = t - a[j + len];
        a[j + len] = montgomery_reduce64((int64_t)zeta * a[j + len]);
      }
    }
  }

  for(j = 0; j < DILITH_N; ++j) {
    a[j] = montgomery_reduce64((int64_t)f * a[j]);
  }
}
