...
**************************************************/
void unpack_pk(uint8_t rho[DILITH_SEEDBYTES],
               polyveck *t1,
             ...
             const polyveck *s2)
{
  ...
  for(i = 0; i < DILITH_CRHBYTES; ++i)
    sk[i] = tr[i];
  sk += DILITH_CRHBYTES;

  for(i = 0; i < DILITH_L; ++i)
    polyeta_pack(sk + i*POLYETA_PACKEDBYTES, &s1->vec[i]);
  sk += DILITH_L*POLYETA_PACKEDBYTES;

  for(i = 0; i < DILITH_K; ++i)
    polyeta_pack(sk + i*POLYETA_PACKEDBYTES, &s2->vec[i]);
  sk += DILITH_K*POLYETA_PACKEDBYTES;
...
*              - const uint8_t *c: pointer to challenge hash length DILITH_SEEDBYTES
*              - const polyvecl *z: pointer to vector z
*              - const polyveck *h: pointer to hint vector h
**************************************************/
void pack_sig(uint8_t sig[DILITHIUM_CRYPTO_BYTES],
              const uint8_t c[DILITH_SEEDBYTES],
              const polyvecl *z,
              const polyveck *h)
{
  unsigned int i, j, k;
  ...
  for(i=0; i < DILITH_SEEDBYTES; ++i)
    sig[i] = c[i];
  sig += DILITH_SEEDBYTES;

  for(i = 0; i < DILITH_L; ++i)
    polyz_pack(sig + i*POLYZ_PACKEDBYTES, &z->vec[i]);
  sig += DILITH_L*POLYZ_PACKEDBYTES;

  /* Encode h */
  for(i = 0; i < OMEGA + DILITH_K; ++i)
    sig[i] = 0;

  k = 0;
  for(i = 0; i < DILITH_K; ++i) {
    for(j = 0; j < DILITH_N; ++j)
...
**************************************************/
int unpack_sig(uint8_t c[DILITH_SEEDBYTES],
               polyvecl *z,
               polyveck *h,
