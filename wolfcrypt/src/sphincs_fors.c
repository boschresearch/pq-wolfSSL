                          ...
                          uint32_t addr_idx, const uint32_t fors_tree_addr[8])
{
    ...
    sphincs_set_type(fors_leaf_addr, SPX_ADDR_TYPE_FORSTREE);
    sphincs_set_tree_index(fors_leaf_addr, addr_idx);
               ...
               const uint32_t fors_addr[8])
{
    ...
    sphincs_set_type(fors_tree_addr, SPX_ADDR_TYPE_FORSTREE);
    sphincs_set_type(fors_pk_addr, SPX_ADDR_TYPE_FORSPK);
    ...
    for (i = 0; i < SPX_FORS_TREES; i++) {
        ...
        sphincs_set_tree_height(fors_tree_addr, 0);
        sphincs_set_tree_index(fors_tree_addr, indices[i] + idx_offset);
