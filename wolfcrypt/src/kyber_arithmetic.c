...
/* Code to generate kyber_zetas and kyber_zetas_inv used in the number-theoretic transform:
...
static int16_t fqmul(int16_t a, int16_t b) {
  return montgomery_reduce32((int32_t)a*b);
}
...
void init_ntt() {
  ...
  tmp[0] = MONT_KYBER;
  for(i = 1; i < 128; ++i)
    tmp[i] = fqmul(tmp[i-1], KYBER_ROOT_OF_UNITY*MONT_KYBER % KYBER_Q);
  ...
  for(i = 64; i >= 1; i >>= 1)
    for(j = i; j < 2*i; ++j)
      kyber_zetas_inv[k++] = -tmp[128 - tree[j]];

  kyber_zetas_inv[127] = MONT_KYBER * (MONT_KYBER * (KYBER_Q - 1) * ((KYBER_Q - 1)/128) % KYBER_Q) % KYBER_Q;
}
...
const int16_t kyber_zetas_inv[128] = {
...
* Name:        kyber_ntt
*
* Description: Inplace number-theoretic transform (kyber_ntt) in Rq
...
*              used for multiplication of elements in Rq in kyber_ntt domain
*
