                  ...
                  const unsigned char *pub_seed, uint32_t addr[8])
{
    ...
    for (i = 0; i < tree_height - 1; i++) {
        ...
        sphincs_set_tree_height(addr, i + 1);
        sphincs_set_tree_index(addr, leaf_idx + idx_offset);
    ...
    leaf_idx >>= 1;
    idx_offset >>= 1;
    sphincs_set_tree_height(addr, tree_height);
    sphincs_set_tree_index(addr, leaf_idx + idx_offset);
              ...
              uint32_t tree_addr[8])
{
    ...
    for (idx = 0; idx < (uint32_t)(1 << tree_height); idx++) {
        ...
        while (offset >= 2 && heights[offset - 1] == heights[offset - 2]) {
            ...
            /* Set the address of the node we're creating. */
            sphincs_set_tree_height(tree_addr, heights[offset - 1] + 1);
            sphincs_set_tree_index(tree_addr,
