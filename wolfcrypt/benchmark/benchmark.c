...
#define BENCH_KYBER_KEYGEN       0x00000111
#define BENCH_KYBER_ENCRYPT      0x00000110
#define BENCH_DILITHIUM_KEYGEN   0x00400001
#define BENCH_DILITHIUM_SIGN     0x00400010
#define BENCH_FALCON_KEYGEN      0x00400100
#define BENCH_FALCON_SIGN        0x00400101
#define BENCH_SPHINCS_KEYGEN     0x00401000
#define BENCH_SPHINCS_SIGN       0x00401001
#define BENCH_XMSS_KEYGEN        0x00410000
#define BENCH_XMSS_SIGN          0x00410001
...
#endif
#ifdef HAVE_KYBER
    { "-kyber-kg",           BENCH_KYBER_KEYGEN      },
    #ifdef HAVE_KYBER_ENCRYPT
    { "-kyber-enc",          BENCH_KYBER_ENCRYPT     },
    #endif
#endif
#ifdef HAVE_DILITHIUM
    { "-dilithium-kg",       BENCH_DILITHIUM_KEYGEN  },
    { "-dilithium",          BENCH_DILITHIUM_SIGN    },
#endif
#ifdef HAVE_FALCON
    { "-falcon-kg",          BENCH_FALCON_KEYGEN     },
    { "-falcon",             BENCH_FALCON_SIGN       },
#endif
#ifdef HAVE_SPHINCS
    { "-sphincs-kg",         BENCH_SPHINCS_KEYGEN    },
    { "-sphincs",            BENCH_SPHINCS_SIGN      },
#endif
#ifdef HAVE_XMSS
    { "-xmss-kg",            BENCH_XMSS_KEYGEN       },
    { "-xmss",              BENCH_XMSS_SIGN         },
#endif
    { NULL, 0}
};
...
static const char* bench_result_words1[][4] = {
    ...
    defined(HAVE_XMSS) || defined(HAVE_KYBER) || defined(HAVE_KYBER_ENCRYPT)
...
/* determine benchmark buffer to use (if NO_FILESYSTEM) */
    ...
    WOLFSSL_API void wolfSSL_Debugging_OFF(void);
                        ...
                        || defined(HAVE_KYBER) || defined(HAVE_DILITHIUM) \
    ...
    #define BENCH_ASYM
           ...
           !defined(NO_DH) || defined(HAVE_KYBER)
...
#define BENCH_MIN_RUNTIME_SEC   10.0f
...
/* use kB instead of mB for embedded benchmarking */
    ...
    enum BenchmarkBounds {
        ...
        genTimes   = 50, /* must be at least BENCH_MAX_PENDING */
        agreeTimes = 200
    };
    ...
    const char* desc, int doAsync, int count, double start, int ret)
{
    ...
    if (csv_format == 1) {
        ...
        XSNPRINTF(msg, sizeof(msg), "%-9s %5d %-11s %s %6d %s %10.3f %s, %s %10.3f ms,"
...
#ifdef HAVE_DILITHIUM
    if (bench_all || (bench_asym_algs & BENCH_DILITHIUM_KEYGEN))
        bench_dilithiumKeyGen();
    if (bench_all || (bench_asym_algs & BENCH_DILITHIUM_SIGN))
        bench_dilithiumKeySign();
#endif

#ifdef HAVE_FALCON
    if (bench_all || (bench_asym_algs & BENCH_FALCON_KEYGEN))
        bench_falconKeyGen();
    if (bench_all || (bench_asym_algs & BENCH_FALCON_SIGN))
        bench_falconKeySign();
#endif

#ifdef HAVE_SPHINCS
    if (bench_all || (bench_asym_algs & BENCH_SPHINCS_KEYGEN))
        bench_sphincsKeyGen();
    if (bench_all || (bench_asym_algs & BENCH_SPHINCS_SIGN))
        bench_sphincsKeySign();
#endif

#ifdef HAVE_XMSS
    if (bench_all || (bench_asym_algs & BENCH_XMSS_KEYGEN))
        bench_xmssKeyGen();
    if (bench_all || (bench_asym_algs & BENCH_XMSS_SIGN))
        bench_xmssKeySign();
#endif

#ifdef HAVE_KYBER
    if (bench_all || (bench_asym_algs & BENCH_KYBER_KEYGEN)) {
    #ifndef NO_SW_BENCH
        bench_kyberKeyGen(0);
    #endif
    #if defined(WOLFSSL_ASYNC_CRYPT) && defined(WC_ASYNC_ENABLE_KYBER) && \
        !defined(NO_HW_BENCH)
        bench_kyberKeyGen(1);
    #endif
    }
    #ifdef HAVE_KYBER_ENCRYPT
    if (bench_all || (bench_asym_algs & BENCH_KYBER_ENCRYPT))
        bench_kyberEncrypt();
    #endif
#endif
...
#ifdef HAVE_DILITHIUM
void bench_dilithiumKeyGen(void)
{
    DilithiumKey genKey;
    double start;
    int    i, count;
    ...
    do {
        for (i = 0; i < genTimes; i++) {
            wc_InitDilithiumKey(&genKey);
            (void)wc_DilithiumKeyGen(&genKey, &gRng);
            wc_FreeDilithiumKey(&genKey);
        }
        count += i;
    } while (bench_stats_sym_check(start));
    bench_stats_asym_finish("Dilithium", DILITHIUM_MODE, desc[2], 0, count, start, 0);
}


void bench_dilithiumKeySign(void)
{
    int    ret;
    DilithiumKey genKey;
#ifdef HAVE_DILITHIUM_SIGN
    double start;
    int    i, count;
    byte   sig[DILITHIUM_SIG_SIZE+512];
    byte   msg[512];
    size_t x = 0;
    size_t y = 0;
#endif
    ...
    wc_InitDilithiumKey(&genKey);
    ret = wc_DilithiumKeyGen(&genKey, &gRng);
    if (ret != 0) {
        printf("wc_DilithiumKeyGen failed\n");
        return;
    }
    ...
    do {
        for (i = 0; i < agreeTimes; i++) {
            x = sizeof(sig);
            ret = wc_DilithiumSign(sig, &x, msg, sizeof(msg), &genKey, &gRng);
            if (ret != 0) {
                printf("wc_DilithiumSign failed\n");
                goto exit_dilithium_sign;
            }
        }
        count += i;
    } while (bench_stats_sym_check(start));
exit_dilithium_sign:
    bench_stats_asym_finish("Dilithium", DILITHIUM_MODE, desc[4], 0, count, start, ret);

#ifdef HAVE_DILITHIUM_VERIFY
    bench_stats_start(&count, &start);
    do {
        for (i = 0; i < agreeTimes; i++) {
            ret = wc_DilithiumVerify(msg, &y, sig, x, &genKey);
            if (ret != 0) {
                printf("wc_DilithiumVerify failed\n");
                goto exit_dilithium_verify;
            }
        }
        count += i;
    } while (bench_stats_sym_check(start));
exit_dilithium_verify:
    bench_stats_asym_finish("Dilithium", DILITHIUM_MODE, desc[5], 0, count, start, ret);
#endif /* HAVE_DILITHIUM_VERIFY */
#endif /* HAVE_DILITHIUM_SIGN */
...
void bench_falconKeyGen(void)
{
    ...
    do {
        for (i = 0; i < genTimes; i++) {
            wc_InitFalconKey(&genKey);
            (void)wc_FalconKeyGen(&genKey, &gRng);
            wc_FreeFalconKey(&genKey);
        }
        count += i;
    } while (bench_stats_sym_check(start));
    bench_stats_asym_finish("Falcon", FALCON_MODE, desc[2], 0, count, start, 0);
}


void bench_falconKeySign(void)
{
    int    ret;
    FalconKey genKey;
#ifdef HAVE_FALCON_SIGN
    double start;
    int    i, count;
    byte   sig[FALCON_SIG_SIZE+512];
    byte   msg[512];
    size_t x = 0;
    size_t y = 0;
#endif
    ...
    wc_InitFalconKey(&genKey);
    ret = wc_FalconKeyGen(&genKey, &gRng);
    if (ret != 0) {
        printf("wc_FalconKeyGen failed\n");
        return;
    }
    ...
    do {
        for (i = 0; i < agreeTimes; i++) {
            x = sizeof(sig);
            ret = wc_FalconSign(sig, &x, msg, sizeof(msg), &genKey, &gRng);
            if (ret != 0) {
                printf("wc_FalconSign failed\n");
                goto exit_falcon_sign;
            }
        }
        count += i;
    } while (bench_stats_sym_check(start));
exit_falcon_sign:
    bench_stats_asym_finish("Falcon", FALCON_MODE, desc[4], 0, count, start, ret);

#ifdef HAVE_FALCON_VERIFY
    bench_stats_start(&count, &start);
    do {
        for (i = 0; i < agreeTimes; i++) {
            ret = wc_FalconVerify(msg, &y, sig, x, &genKey);
            if (ret != 0) {
                printf("wc_FalconVerify failed\n");
                goto exit_falcon_verify;
            }
        }
        count += i;
    } while (bench_stats_sym_check(start));
exit_falcon_verify:
    bench_stats_asym_finish("Falcon", FALCON_MODE, desc[5], 0, count, start, ret);
#endif /* HAVE_FALCON_VERIFY */
#endif /* HAVE_FALCON_SIGN */
...
void bench_sphincsKeyGen(void)
{
    ...
    do {
        for (i = 0; i < genTimes; i++) {
            wc_InitSphincsKey(&genKey);
            (void)wc_SphincsKeyGen(&genKey, &gRng);
            wc_FreeSphincsKey(&genKey);
        }
        count += i;
    } while (bench_stats_sym_check(start));
    bench_stats_asym_finish("Sphincs", SPX_MODE, desc[2], 0, count, start, 0);
}


void bench_sphincsKeySign(void)
{
    int    ret;
    SphincsKey genKey;
#ifdef HAVE_SPHINCS_SIGN
    double start;
    int    i, count;
    byte   sig[SPHINCS_SIG_SIZE+512];
    byte   msg[512];
    ...
    wc_InitSphincsKey(&genKey);
    ret = wc_SphincsKeyGen(&genKey, &gRng);
    if (ret != 0) {
        printf("wc_SphincsKeyGen failed\n");
        return;
    }
    ...
    do {
        for (i = 0; i < agreeTimes; i++) {
            x = sizeof(sig);
            ret = wc_SphincsSign(sig, &x, msg, sizeof(msg), &genKey, &gRng);
            if (ret != 0) {
                printf("wc_SphincsSign failed\n");
                goto exit_sphincs_sign;
            }
        }
        count += i;
    } while (bench_stats_sym_check(start));
exit_sphincs_sign:
    bench_stats_asym_finish("Sphincs", SPX_MODE, desc[4], 0, count, start, ret);

#ifdef HAVE_SPHINCS_VERIFY
    bench_stats_start(&count, &start);
    do {
        for (i = 0; i < agreeTimes; i++) {
            ret = wc_SphincsVerify(msg, &y, sig, x, &genKey);
            if (ret != 0) {
                printf("wc_SphincsVerify failed\n");
                goto exit_sphincs_verify;
            }
        }
        count += i;
    } while (bench_stats_sym_check(start));
exit_sphincs_verify:
    bench_stats_asym_finish("Sphincs", SPX_MODE, desc[5], 0, count, start, ret);
#endif /* HAVE_SPHINCS_VERIFY */
#endif /* HAVE_SPHINCS_SIGN */
...
void bench_xmssKeyGen(void)
{
    ...
    do {
        for (i = 0; i < genTimes; i++) {
            wc_InitXmssKey(&genKey);
            (void)wc_XmssKeyGen(&genKey, &gRng);
            wc_FreeXmssKey(&genKey);
        }
        count += i;
    } while (bench_stats_sym_check(start));
    bench_stats_asym_finish("XMSS", XMSS_MODE, desc[2], 0, count, start, 0);
}


void bench_xmssKeySign(void)
{
    int    ret;
    XmssKey genKey;
#ifdef HAVE_XMSS_SIGN
    double start;
    int    i, count;
    byte   sig[XMSS_SIG_SIZE+512];
    byte   msg[512];
    long long unsigned int x = 0;
    long long unsigned int y = 0;
#endif
    ...
    wc_InitXmssKey(&genKey);
    ret = wc_XmssKeyGen(&genKey, &gRng);
    if (ret != 0) {
        printf("wc_XmssKeyGen failed\n");
        return;
    }
    ...
    do {
        for (i = 0; i < agreeTimes; i++) {
            x = sizeof(sig);
            ret = wc_XmssSign(sig, &x, msg, sizeof(msg), &genKey);
            if (ret != 0) {
                printf("wc_XmssSign failed\n");
                goto exit_xmss_sign;
            }
        }
        count += i;
...
exit_xmss_sign:
    bench_stats_asym_finish("XMSS", XMSS_MODE, desc[4], 0, count, start, ret);

#ifdef HAVE_XMSS_VERIFY
    bench_stats_start(&count, &start);
    do {
        for (i = 0; i < agreeTimes; i++) {
            ret = wc_XmssVerify(msg, &y, sig, x, &genKey);
            if (ret != 0) {
                printf("wc_XmssVerify failed\n");
                goto exit_xmss_verify;
            }
        }
        count += i;
    } while (bench_stats_sym_check(start));
exit_xmss_verify:
    bench_stats_asym_finish("XMSS", XMSS_MODE, desc[5], 0, count, start, ret);
#endif /* HAVE_XMSS_VERIFY */
#endif /* HAVE_XMSS_SIGN */
...
void bench_kyberKeyGen(int doAsync)
{
    int ret = 0, i, times, count, pending = 0;
    KyberKey genKey[BENCH_MAX_PENDING];
    double start;
    const char**desc = bench_desc_words[lng_index];
        ...
        for (times = 0; times < genTimes || pending > 0; ) {
            ...
            for (i = 0; i < BENCH_MAX_PENDING; i++) {
                if (bench_async_check(&ret, BENCH_ASYNC_GET_DEV(&genKey[i]), 0, &times, genTimes, &pending)) {
                    ...
                    wc_FreeKyberKey(&genKey[i]);
                    ret = wc_InitKyberKey(&genKey[i]);
                    if (ret < 0) {
                        goto exit;
                    }

                    ret = wc_GenerateKyberKeyPair(&genKey[i], &gRng);
                    if (!bench_async_handle(&ret, BENCH_ASYNC_GET_DEV(&genKey[i]), 0, &times, &pending)) {
                        goto exit;
                    }
                }
...
exit:
    bench_stats_asym_finish("Kyber",KYBER_MODE, desc[2], doAsync, count, start, ret);
...
#ifdef HAVE_KYBER_ENCRYPT
void bench_kyberEncrypt(void)
{
    KyberKey userA, userB;
    int     ret, i, count;
    double start;
    const char**desc = bench_desc_words[lng_index];

    ret = wc_InitKyberKey(&userA);
    if (ret != 0) {
        printf("wc_InitKyberKey make key A failed: %d\n", ret);
        return;
    }

    ret = wc_InitKyberKey(&userB);
    if (ret != 0) {
        printf("wc_InitKyberKey make key B failed: %d\n", ret);
        return;
    }

    ret = wc_GenerateKyberKeyPair(&userA, &gRng);
    if (ret != 0)
        goto exit;
    ...
    do {
        for (i = 0; i < ntimes; i++) {
            /* encrypt msg to B */
            ret = wc_KyberEncrypt(&userA, &gRng);
            if (ret != 0) {
                printf("wc_KyberEncrypt failed! %d\n", ret);
                goto exit_enc;
            }
        }
        count += i;
    ...
    XMEMCPY(userB.ct, userA.ct, KYBER_CIPHERTEXTBYTES);
    XMEMCPY(userB.priv, userA.priv, KYBER_SECRETKEYBYTES);
exit_enc:
    bench_stats_asym_finish("Kyber", KYBER_MODE, desc[6], 0, count, start, ret);

    bench_stats_start(&count, &start);
    do {
        for (i = 0; i < ntimes; i++) {
            /* decrypt msg from A */
            wc_KyberDecrypt(&userB);
            ret = memcmp(&userA.ss, &userB.ss, sizeof(userA.ss));
            if (ret != 0) {
                printf("wc_KyberDecrypt failed! %d\n", ret);
                goto exit_dec;
            }
        }
        count += i;
...
exit_dec:
    bench_stats_asym_finish("Kyber", KYBER_MODE, desc[7], 0, count, start, ret);

exit:

    /* cleanup */
    wc_FreeKyberKey(&userB);
    wc_FreeKyberKey(&userA);
}
#endif
#endif /* HAVE_KYBER */
    ...
    #include <sys/time.h>
    /* Measure time on PC, implementation inspired by code from tls_bench.c */
    double current_time(int reset)
    {
        ...
        return ((double)tspec.tv_sec) + ((double)tspec.tv_nsec / 1000000000L);
    }
#else
