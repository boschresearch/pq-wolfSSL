                        ...
                        int useX448, int useKyber)
{
    ...
    if (onlyKeyShare == 0 || onlyKeyShare == 2) {
        ...
        if (useKyber) {
            ...
            groups[count++] = WOLFSSL_KYBER_512;
            if (wolfSSL_UseKeyShare(ssl, WOLFSSL_KYBER_512) != WOLFSSL_SUCCESS)
                err_sys("unable to use Kyber 512");
        #elif defined(HAVE_KYBER_768)
            groups[count++] = WOLFSSL_KYBER_768;
            if (wolfSSL_UseKeyShare(ssl, WOLFSSL_KYBER_768) != WOLFSSL_SUCCESS)
                err_sys("unable to use Kyber 768");
        #elif defined(HAVE_KYBER_1024)
            groups[count++] = WOLFSSL_KYBER_1024;
            if (wolfSSL_UseKeyShare(ssl, WOLFSSL_KYBER_1024) != WOLFSSL_SUCCESS)
                err_sys("unable to use Kyber 1024");
        #endif
        }
        else
    #endif
...
static int ClientBenchmarkConnections(WOLFSSL_CTX* ctx, char* host, word16 port,
    ...
    int useX448, int helloRetry, int onlyKeyShare, int version, int earlyData, int useKyber)
{
    ...
    while (loops--) {
        ...
        for (i = 0; i < times; i++) {
            ...
            if (benchResume)
                ...
                if (!helloRetry)
                    SetKeyShare(ssl, onlyKeyShare, useX25519, useX448, useKyber);
                else
...
static int ClientBenchmarkThroughput(WOLFSSL_CTX* ctx, char* host, word16 port,
    ...
    int useX448, int exitWithRet, int useKyber)
{
    ...
    (void)useKyber;
            ...
            if (useKyber) {
            #if defined(HAVE_KYBER_512)
                if (wolfSSL_UseKeyShare(ssl, WOLFSSL_KYBER_512)
                        != WOLFSSL_SUCCESS) {
                    err_sys("unable to use Kyber 512");
                }
            #elif defined(HAVE_KYBER_768)
                if (wolfSSL_UseKeyShare(ssl, WOLFSSL_KYBER_768)
                        != WOLFSSL_SUCCESS) {
                    err_sys("unable to use Kyber 768");
                }
            #elif defined(HAVE_KYBER_1024)
                if (wolfSSL_UseKeyShare(ssl, WOLFSSL_KYBER_1024)
                        != WOLFSSL_SUCCESS) {
                    err_sys("unable to use Kyber 1024");
                }
            #endif
 ...
 || defined(HAVE_CERTIFICATE_STATUS_REQUEST_V2)
    ...
    while ((ch = mygetopt(argc, argv, "?:"
        ...
        switch (ch) {
            ...
            case '9' :
                #if defined(WOLFSSL_TLS13) && defined(HAVE_KYBER)
                    useKyber = 1;
                    onlyKeyShare = 2;
                #endif
                break;
    ...
    if (!usePsk) {
        usePsk = 1;
                                                ...
                                                !defined(HAVE_KYBER) && !defined(HAVE_ED448)
    if (!usePsk) {
        usePsk = 1;
    }
#endif
    ...
    if (!helloRetry && version >= 4) {
                             ...
                             defined(HAVE_CURVE25519) || defined(HAVE_CURVE448)) || defined(HAVE_KYBER)
        if (onlyKeyShare == 0 || onlyKeyShare == 2) {
        #ifdef HAVE_CURVE25519
            if (useX25519) {
                if (wolfSSL_UseKeyShare(ssl, WOLFSSL_ECC_X25519)
