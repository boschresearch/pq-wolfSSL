...
enum Misc {
...
#else
    #if defined(HAVE_SPHINCS) || defined(HAVE_XMSS)
    ENCRYPT_LEN     = 40000,
    #elif defined(HAVE_DILITHIUM) || defined(HAVE_FALCON)
    ENCRYPT_LEN     = 5000,     /* allow 40000 bit static buffer */
    #else
    ...
    NAMED_KYBER_MASK = 0xFE00, /* Named group mask for Kyber parameters */
    MIN_KYBER_GROUP = 0xFE00,  /* Named group minimum for Kyber parameters */
    MAX_KYBER_GROUP = 0xFEFF,  /* Named group maximum for Kyber parameters */
...
#if defined(HAVE_SPHINCS) || defined(HAVE_XMSS)
    MAX_CERT_VERIFY_SZ = 60000,  /* max Sphincs */
#elif defined(HAVE_DILITHIUM) || defined(HAVE_FALCON)
    MAX_CERT_VERIFY_SZ = 20000, /* max Dilithium/Falcon */
#elif !defined(NO_RSA)
...
#if defined(HAVE_SPHINCS) || defined(HAVE_XMSS)
    MAX_X509_SIZE      = 60000, /* max static x509 buffer size */
#elif defined(HAVE_DILITHIUM) || defined(HAVE_FALCON)
    MAX_X509_SIZE      = 20000, /* max static x509 buffer size */
#else
...
/* set minimum Dilithium key size allowed */
#define MIN_DILITHIUMKEY_SZ 1312

/* set minimum Falcon key size allowed */
#define MIN_FALCONKEY_SZ    897

/* set minimum Sphincs key size allowed */
#define MIN_SPHINCSKEY_SZ    32

/* set minimum XMSS key size allowed */
#define MIN_XMSSKEY_SZ       64
...
WOLFSSL_LOCAL void InitSuitesHashSigAlgo(Suites* suites, int haveECDSAsig,
                                         int haveRSAsig, int haveDilithiumSig,
                                         int haveFalconSig, int haveSphincsSig,
                                         int haveXmssSig, int haveAnon,
...
#endif
#ifdef HAVE_SPHINCS
    short           minSphincsKeySz;     /* minimum allowed Sphincs key size */
#endif
#ifdef HAVE_XMSS
    short           minXmssKeySz;     /* minimum allowed XMSS key size */
#endif
};
...
struct WOLFSSL_CTX {
    ...
    byte        haveSphincsSig:1;  /* server cert signed w/ Sphincs */
    byte        haveXmssSig:1;     /* server cert signed w/ XMSS */
...
#endif
#ifdef HAVE_SPHINCS
    short       minSphincsKeySz;  /* minimum Sphincs key size */
#endif
#ifdef HAVE_XMSS
    short       minXmssKeySz;     /* minimum XMSS key size */
#endif
...
typedef struct Options {
    ...
    word16            haveSphincsSig:1;   /* server Sphincs signed cert */
    word16            haveXmssSig:1;      /* server XMSS signed cert */
...
#if defined(HAVE_KYBER) || defined(HAVE_DILITHIUM) || defined(HAVE_FALCON) || \
    defined(HAVE_SPHINCS) || defined(HAVE_XMSS) || defined(HAVE_ECC)
typedef struct PqInfo {
    /* size of TLS extensions */
    word32          supportedGroupsSz;
    word32          keyShareSz;
    word32          signatureAlgsSz;
    word32          signatureAlgsCertSz;

    /* size of handshake messages */
    word32          clientHelloSz;
    word32          serverHelloSz;
    word32          clientCertificateSz;
    word32          serverCertificateSz;
    word32          certificateRequestSz;
    word32          clientCertificateVerifySz;
    word32          serverCertificateVerifySz;

    /* measurements for Client */
    double          clientStart;
    double          clientEnd;
    double          clientGotReply;

    /* measurements for Server */
    double          serverStart;
    double          serverEnd;
} PqInfo;
#endif
...
#endif
#ifdef HAVE_DILITHIUM
    DilithiumKey*   peerDilithiumKey;
    byte            peerDilithiumKeyPresent;
#endif
#ifdef HAVE_FALCON
    FalconKey*      peerFalconKey;
    byte            peerFalconKeyPresent;
#endif
#ifdef HAVE_SPHINCS
    SphincsKey*     peerSphincsKey;
    byte            peerSphincsKeyPresent;
#endif
#ifdef HAVE_XMSS
    XmssKey*        peerXmssKey;
    byte            peerXmssKeyPresent;
#endif
#ifdef HAVE_KYBER
    KyberKey*       peerKyberKey;
    byte            kyberPublicKeyPresent;    /* if yes, server can calculate shared secret */
#endif
#if defined(HAVE_KYBER) || defined(HAVE_DILITHIUM) || defined(HAVE_FALCON) || \
    defined(HAVE_SPHINCS) || defined(HAVE_XMSS) || defined(HAVE_ECC)
    PqInfo          pqInfo;                   /* info saved during handshake */
#endif
