...
int TLSX_ValidateSupportedCurves(WOLFSSL* ssl, byte first, byte second) {
    ...
    word32          oid       = 0;
                              ...
                              defined(HAVE_CURVE448) || defined(HAVE_ED448) || defined(HAVE_KYBER) || \
                                (!defined(NO_RSA) && defined(WOLFSSL_STATIC_DH))
    word32          pkOid     = 0;
#endif /* HAVE_ECC || HAVE_ED25519 || HAVE_ED448 || HAVE_KYBER || (!NO_RSA && STATIC_DH) */
    word32          defOid    = 0;
    ...
    for (curve = (SupportedCurve*)extension->data;
        ...
        switch (curve->name) {
            ...
            case WOLFSSL_KYBER_512:
                pkOid = oid = KYBER_512_OID;
                octets = 148;
                break;

            case WOLFSSL_KYBER_768:
                pkOid = oid = KYBER_768_OID;
                octets = 148;
                break;

            case WOLFSSL_KYBER_1024:
                pkOid = oid = KYBER_1024_OID;
                octets = 148;
                break;
        #endif /* HAVE_KYBER */
        #if defined(HAVE_DILITHIUM)
            case WOLFSSL_DILITHIUM:
                pkOid = oid = DILITHIUM_OID;
                octets = 148;
                break;
        #endif /* HAVE_DILITHIUM */
        #if defined(HAVE_FALCON)
            case WOLFSSL_FALCON:
                pkOid = oid = FALCON_OID;
                octets = 148;
                break;
        #endif /* HAVE_FALCON */
        #if defined(HAVE_SPHINCS)
            case WOLFSSL_SPHINCS:
                pkOid = oid = SPHINCS_OID;
                octets = 148;
                break;
        #endif /* HAVE_SPHINCS */
...
/* Encapsulates shared Kyber secret (Server side key share).
 *
 * ssl          The SSL/TLS object.
 * kse          The key share entry object.
 * returns 0 on success, otherwise failure.
 */
static int TLSX_KeyShare_KyberEncaps(WOLFSSL *ssl,
                                    KeyShareEntry *kse)
{
    int ret = 0;
    ...
    WOLFSSL_ENTER("TLSX_KeyShare_KyberEncaps()");

    /* Check if client's key is present */
    if (kse->pubKey == NULL) {
        WOLFSSL_MSG("I am the WOLFSSL server. I have not received CLIENT_HELLO yet. Too early to compute Kyber key share.");
        return ret;
    }

    byte* pubKey         = NULL;
    byte*     pubKeyPtr  = NULL;
    KyberKey* privKey    = NULL;
    byte*     privKeyPtr = NULL;

    /* Allocate space for private key. */
    privKeyPtr = (byte*)XMALLOC(sizeof(KyberKey), ssl->heap,
                                            DYNAMIC_TYPE_PRIVATE_KEY);

    if (privKeyPtr == NULL) {
        WOLFSSL_MSG("Kyber Private Key Memory error");
        return MEMORY_E;
    }
    privKey = (KyberKey*)privKeyPtr;
    ret = wc_InitKyberKey(privKey);
    if (ret != 0)
        goto end;

    /* Allocate space for public key */
    pubKeyPtr = (byte*)XMALLOC(KYBER_CIPHERTEXTBYTES, ssl->heap,
                                            DYNAMIC_TYPE_PUBLIC_KEY);

    if (pubKeyPtr == NULL) {
        WOLFSSL_MSG("Kyber Public Key Memory error");
        ret = MEMORY_E;
        goto end;
    }
    pubKey = (byte*)pubKeyPtr;
    XMEMSET(pubKey, 0, sizeof(*pubKey));

    /* Server: calculate shared secret */
    /* Copy peer's public key */
    XMEMCPY(privKey->pub, kse->pubKey, KYBER_PUBLICKEYBYTES);

    /* Calculate shared secret */
    ret = wc_KyberEncrypt(privKey, ssl->rng);
    if (ret != 0)
        goto end;

#ifdef WOLFSSL_DEBUG_PQC
    WOLFSSL_MSG("I am the WOLFSSL Server. I have generated and encrypted Kyber Shared secret");
    WOLFSSL_MSG("Public key: ");
    WOLFSSL_BUFFER(privKey->pub, KYBER_PUBLICKEYBYTES);
    WOLFSSL_MSG("Private key (should be empty): ");
    WOLFSSL_BUFFER(privKey->priv, KYBER_SECRETKEYBYTES);
    WOLFSSL_MSG("Encapsulated shared secret aka ciphertext: ");
    WOLFSSL_BUFFER(privKey->ct, KYBER_CIPHERTEXTBYTES);
    WOLFSSL_MSG("Shared secret: ");
    WOLFSSL_BUFFER(privKey->ss, KYBER_SSBYTES);
#endif /* WOLFSSL_DEBUG_PQC */

    /* Export server key share (this is the encrypted shared secret) */
    XMEMCPY(pubKey, privKey->ct, KYBER_CIPHERTEXTBYTES);

    /* Store everything into kse */
    kse->pubKey = pubKeyPtr;
    kse->pubKeyLen = KYBER_CIPHERTEXTBYTES;
    kse->key = privKeyPtr;
    kse->keyLen = sizeof(KyberKey);

#ifdef WOLFSSL_DEBUG_PQC
    WOLFSSL_MSG("Kyber key share (encapsulated shared secret)");
    WOLFSSL_BUFFER(pubKey, KYBER_CIPHERTEXTBYTES);
#endif
    ...
    if (ret != 0) {
        /* Data owned by key share entry otherwise. */
        if (pubKeyPtr != NULL)
            XFREE(pubKeyPtr, ssl->heap, DYNAMIC_TYPE_PUBLIC_KEY);
        XFREE(privKeyPtr, ssl->heap, DYNAMIC_TYPE_PRIVATE_KEY);
        pubKey = NULL;
        privKey = NULL;
        pubKeyPtr = NULL;
        privKeyPtr = NULL;
    }
#else /* NO KYBER DEFINED */
    (void)ssl;
    (void)kse;
    ...
    WOLFSSL_LEAVE("TLSX_KeyShare_KyberEncaps()", ret);
    return ret;
}

/* Generates kyber (pk, sk) pair (Client side key share).
 *
 * ssl          The SSL/TLS object.
 * kse    The client key share entry object.
 * returns 0 on success, otherwise failure.
 */
static int TLSX_KeyShare_KyberKeyGen(WOLFSSL *ssl, KeyShareEntry *kse)
{
    int ret = 0;

#ifdef HAVE_KYBER

    WOLFSSL_ENTER("TLSX_KeyShare_KyberKeyGen()");
    ...
    /* Allocate space for public key */
    pubKeyPtr = (byte*)XMALLOC(KYBER_PUBLICKEYBYTES, ssl->heap,
                                            DYNAMIC_TYPE_PUBLIC_KEY);

    if (pubKeyPtr == NULL) {
        WOLFSSL_MSG("Key Public Key Memory error");
        ret = MEMORY_E;
        goto end;
    }
    pubKey = (byte*)pubKeyPtr;
    ...
    ret = wc_GenerateKyberKeyPair(privKey, ssl->rng);
    if (ret != 0)
        goto end;

#ifdef WOLFSSL_DEBUG_PQC
    WOLFSSL_MSG("I am the WOLFSSL client. I have generated the Kyber Key pair.");
    WOLFSSL_MSG("Public Kyber Key: ");
    WOLFSSL_BUFFER(privKey->pub, KYBER_PUBLICKEYBYTES);
    WOLFSSL_MSG("Private Kyber Key: ");
    WOLFSSL_BUFFER(privKey->priv, KYBER_SECRETKEYBYTES);
    WOLFSSL_MSG("Kyber Ciphertext (should be empty): ");
    WOLFSSL_BUFFER(privKey->ct, KYBER_CIPHERTEXTBYTES);
    WOLFSSL_MSG("Kyber Shared Secret (should be empty): ");
    WOLFSSL_BUFFER(privKey->ss, KYBER_SSBYTES);
#endif /* WOLFSSL_DEBUG_PQC */

    /* Export public key. */
    XMEMCPY(pubKey, privKey->pub, KYBER_PUBLICKEYBYTES);

    /* Store everything into kse */
    kse->pubKey = pubKeyPtr;
    kse->pubKeyLen = KYBER_PUBLICKEYBYTES;
    kse->key = privKeyPtr;
    kse->keyLen = sizeof(KyberKey);

#ifdef WOLFSSL_DEBUG_PQC
    WOLFSSL_MSG("Kyber Public Key");
    WOLFSSL_BUFFER(pubKey, KYBER_PUBLICKEYBYTES);
#endif
    ...
    WOLFSSL_LEAVE("TLSX_KeyShare_KyberKeyGen()", ret);
    return ret;
}
...
static int TLSX_KeyShare_GenKey(WOLFSSL *ssl, KeyShareEntry *kse)
{
    ...
    if (kse->group & NAMED_KYBER_MASK) {
        /* Client end: Kyber key generation */
        if (ssl->options.side == WOLFSSL_CLIENT_END)
            return TLSX_KeyShare_KyberKeyGen(ssl, kse);

        /* Server end: Encapsulate shared secret */
        if (ssl->options.side == WOLFSSL_SERVER_END)
            return TLSX_KeyShare_KyberEncaps(ssl, kse);
    }
...
static void TLSX_KeyShare_FreeAll(KeyShareEntry* list, void* heap)
{
    ...
    while ((current = list) != NULL) {
        ...
        if ((current->group & NAMED_DH_MASK) == 0) {
            ...
            else if (current->group & NAMED_KYBER_MASK) {
#ifdef HAVE_KYBER
                wc_FreeKyberKey((KyberKey*)(current->key));
#endif
            }
            else {
#ifdef HAVE_ECC
...
static word16 TLSX_KeyShare_Write(KeyShareEntry* list, byte* output,
                                  byte msgType)
{
    WOLFSSL_ENTER("TLSX_KeyShare_Write()");
    ...
    while ((current = list) != NULL) {
        ...
        if (!isRequest && current->key == NULL) {
            WOLFSSL_MSG("I am the WOLFSSL server and my private key is empty. I cannot write the Key Share in the HELLO.");
            continue;
        }
        WOLFSSL_MSG("I am writing the Key Share entry in the HELLO.");
...
/* Kyber decapsulation: process the Kyber key share extension on the client side.
 *
 * ssl            The SSL/TLS object.
 * keyShareEntry  The key share entry object to use to calculate shared secret.
 * returns 0 on success and other values indicate failure.
 */
static int TLSX_KeyShare_ProcessKyber(WOLFSSL* ssl, KeyShareEntry* keyShareEntry)
{
    int ret = 0;

#ifdef HAVE_KYBER

    WOLFSSL_ENTER("TLSX_KeyShare_ProcessKyber()");

    KyberKey* keyShareKey = (KyberKey*)keyShareEntry->key;
    byte* encapsKey = (byte*)keyShareEntry->ke;

    /* Do decapsulation only on client; server has already calculated its shared secret */

    if (ssl->options.side == WOLFSSL_CLIENT_END) {
        /* Check if encapsulated shared secret is present */
        if (encapsKey == NULL) {
            WOLFSSL_MSG("KyberEncaps Key missing error");
            return KYBER_KEY_ERROR;
        }

        /* copy ciphertext into keyShareKey */
        XMEMCPY(keyShareKey->ct, encapsKey, KYBER_CIPHERTEXTBYTES);

        ret = wc_KyberDecrypt(keyShareKey);
        if (ret != 0)
            goto end;

        #ifdef WOLFSSL_DEBUG_PQC
            WOLFSSL_MSG("I am the WOLFSSL client. I have the server Key Share. I have computed the Shared Secret.");
            WOLFSSL_MSG("Public Kyber Key: ");
            WOLFSSL_BUFFER(keyShareKey->pub, KYBER_PUBLICKEYBYTES);
            WOLFSSL_MSG("Private Kyber Key: ");
            WOLFSSL_BUFFER(keyShareKey->priv, KYBER_SECRETKEYBYTES);
            WOLFSSL_MSG("Encapsulated Shared Secret aka Ciphertext: ");
            WOLFSSL_BUFFER(keyShareKey->ct, KYBER_CIPHERTEXTBYTES);
            WOLFSSL_MSG("Shared Secret: ");
            WOLFSSL_BUFFER(keyShareKey->ss, KYBER_SSBYTES);
        #endif
    }
    XMEMSET(ssl->arrays->preMasterSecret, 0,  ssl->arrays->preMasterSz);
    XMEMCPY(ssl->arrays->preMasterSecret, keyShareKey->ss, KYBER_SSBYTES);

    #ifdef WOLFSSL_DEBUG_PQC
        WOLFSSL_MSG("Premaster Secret is the Shared Secret:");
    ...
    if (ret != 0) {
       ...
       XFREE(encapsKey, ssl->heap, DYNAMIC_TYPE_PUBLIC_KEY);
        keyShareEntry->pubKey = NULL;
    }
#else
    (void)ssl;
...
{
    WOLFSSL_ENTER("TLSX_KeyShare_Process()");
    int ret = 0;
    ...
    else if (keyShareEntry->group & NAMED_KYBER_MASK)
        ret = TLSX_KeyShare_ProcessKyber(ssl, keyShareEntry);
    else
        ret = TLSX_KeyShare_ProcessEcc(ssl, keyShareEntry);

#if defined(WOLFSSL_DEBUG_TLS) || defined(WOLFSSL_DEBUG_PQC)
    WOLFSSL_MSG("KE Secret");
    WOLFSSL_BUFFER(ssl->arrays->preMasterSecret, ssl->arrays->preMasterSz);
#endif

    WOLFSSL_LEAVE("TLSX_KeyShare_Process()", ret);
    return ret;
}
...
{
#ifdef HAVE_SUPPORTED_CURVES
    WOLFSSL_ENTER("TLSX_SupportedGroups_Find()");
    ...
    if ((extension = TLSX_Find(ssl->extensions,
        ...
        if ((extension = TLSX_Find(ssl->ctx->extensions,
                                              TLSX_SUPPORTED_GROUPS)) == NULL) {
            WOLFSSL_MSG("I failed to find Group name.");
            return 0;
        }
    }
...
static int TLSX_KeyShare_Parse(WOLFSSL* ssl, byte* input, word16 length,
                               byte msgType)
{
    WOLFSSL_ENTER("TLSX_KeyShare_Parse()");
    ...
    if (msgType == client_hello) {
        ...
        WOLFSSL_MSG("I am the WOLFSSL server, I am processing CLIENT_HELLO Key Share");
        ...
        extension = TLSX_Find(ssl->extensions, TLSX_KEY_SHARE);
        if (extension == NULL) {
            WOLFSSL_MSG("I cannot find a Key Share extension; creating new one.");
        ...
        WOLFSSL_MSG("I am the WOLFSSL client. I am processing the SERVER_HELLO Key Share.");
    ...
    WOLFSSL_LEAVE("TLSX_KeyShare_Parse()", ret);
...
static int TLSX_KeyShare_IsSupported(int namedGroup)
{
    switch (namedGroup) {
        ...
        case WOLFSSL_FFDHE_8192:
            break;
    #endif
    #ifdef HAVE_KYBER_512
        case WOLFSSL_KYBER_512:
            break;
    #endif
    #ifdef HAVE_KYBER_768
        case WOLFSSL_KYBER_768:
            break;
    #endif
    #ifdef HAVE_KYBER_1024
        case WOLFSSL_KYBER_1024:
            break;
    #endif
    #if (!defined(NO_ECC256)  || defined(HAVE_ALL_CURVES)) && ECC_MIN_KEY_SZ <= 256
...
static int TLSX_KeyShare_GroupRank(WOLFSSL* ssl, int group)
{
    ...
    #if defined(HAVE_KYBER)
        #ifdef HAVE_KYBER_512
            ssl->group[ssl->numGroups++] = WOLFSSL_KYBER_512;
        #endif
        #ifdef HAVE_KYBER_768
            ssl->group[ssl->numGroups++] = WOLFSSL_KYBER_768;
        #endif
        #ifdef HAVE_KYBER_1024
            ssl->group[ssl->numGroups++] = WOLFSSL_KYBER_1024;
        #endif
    #endif /* HAVE_KYBER */
    }
...
int TLSX_KeyShare_Establish(WOLFSSL *ssl)
{
    ...
    WOLFSSL_ENTER("TLSX_KeyShare_Establish()");
    ...
    for (clientKSE = list; clientKSE != NULL; clientKSE = clientKSE->next) {
        ...
        if (!TLSX_SupportedGroups_Find(ssl, clientKSE->group)) {
            WOLFSSL_MSG("I am the WOLFSSL server. I cannot find the client's Supported Group.");
            return BAD_KEY_SHARE_DATA;
        }
        /* fprintf(stderr, "The client's Supported Group is %d\n", clientKSE->group); */

        if (((clientKSE->group & NAMED_DH_MASK) == 0) &&
            ((clientKSE->group & NAMED_KYBER_MASK) == 0)) {
        ...
        if (!TLSX_KeyShare_IsSupported(clientKSE->group)){
            WOLFSSL_MSG("I don't support the client's Group!");
            continue;
        }
    ...
    if (clientKSE == NULL) {
        WOLFSSL_MSG("I could not find client's Supported Groups. Sending HelloRetryRequest.");
    ...
    if (clientKSE->key == NULL) {
        if (clientKSE->group & NAMED_KYBER_MASK) {
            WOLFSSL_MSG("I am the WOLFSSL server. I use Kyber Group.");
            if (clientKSE->ke != NULL) {
                /* for kyber need to copy client's public key */
                WOLFSSL_MSG("I have client's Kyber Public Key.");
                serverKSE->pubKey = clientKSE->ke;
                serverKSE->pubKeyLen = clientKSE->keLen;
            }
            else {
                WOLFSSL_MSG("I don't have client's Kyber Public Key.");
            }
        }
        ret = TLSX_KeyShare_GenKey(ssl, serverKSE);
        if (ret != 0) {
            WOLFSSL_MSG("Failed to generate Key Share.");
            return ret;
        }
    }
    else {
    ...
    WOLFSSL_LEAVE("TLSX_KeyShare_Establish()", ret);
    ...
    while ((extension = list)) {
        ...
        switch (extension->type) {
            ...
            case TLSX_SUPPORTED_GROUPS:
                ...
                /* PQ info */
                pqInfo->supportedGroupsSz = EC_GET_SIZE((SupportedCurve*)extension->data);
                break;
            ...
            case TLSX_SIGNATURE_ALGORITHMS:
                ...
                /* PQ info */
                pqInfo->signatureAlgsSz = SA_GET_SIZE(extension->data);
                break;
#endif
#if defined(HAVE_ENCRYPT_THEN_MAC) && !defined(WOLFSSL_AEAD_ONLY)
            case TLSX_ENCRYPT_THEN_MAC:
            ...
            case TLSX_SIGNATURE_ALGORITHMS_CERT:
                ...
                /* PQ evaluation */
                pqInfo->signatureAlgsCertSz = SAC_GET_SIZE(extension->data);
                break;
    #endif
            ...
            case TLSX_KEY_SHARE:
                ...
                /* PQ evaluation */
                pqInfo->keyShareSz = KS_GET_SIZE((KeyShareEntry*)extension->data, msgType);
                break;
#endif
        }
...
static int TLSX_PopulateSupportedGroups(WOLFSSL* ssl, TLSX** extensions)
{
    ...
    if (ssl->numGroups != 0) {
        ...
        /* Add Kyber supported groups */
        #if defined(HAVE_KYBER)
            #ifdef HAVE_KYBER_512
                ret = TLSX_UseSupportedCurve(extensions,
                                              WOLFSSL_KYBER_512, ssl->heap);
                                              WOLFSSL_MSG("Added KYBER_512 Group.");
                if (ret != WOLFSSL_SUCCESS) {WOLFSSL_MSG("Adding KYBER_512 Group failed."); return ret;}
            #endif
            #ifdef HAVE_KYBER_768
                ret = TLSX_UseSupportedCurve(extensions,
                                              WOLFSSL_KYBER_768, ssl->heap);
                                              WOLFSSL_MSG("Added KYBER_768 Group.");
                if (ret != WOLFSSL_SUCCESS) {WOLFSSL_MSG("Adding KYBER_768 Group failed."); return ret;}
            #endif
            #ifdef HAVE_KYBER_1024
                ret = TLSX_UseSupportedCurve(extensions,
                                              WOLFSSL_KYBER_1024, ssl->heap);
                                              WOLFSSL_MSG("Added KYBER_1024 Group.");
                if (ret != WOLFSSL_SUCCESS) {WOLFSSL_MSG("Adding KYBER_1024 Group failed."); return ret;}
            #endif
        #endif /* HAVE KYBER */
...
#endif
#if defined(HAVE_KYBER)
    #if defined(HAVE_KYBER_512)
        WOLFSSL_KYBER_512,
    #elif defined(HAVE_KYBER_768)
        WOLFSSL_KYBER_768,
    #elif defined(HAVE_KYBER_1024)
        WOLFSSL_KYBER_1024,
    #endif
#endif
};
...
#endif
    if (ssl->extensions) {
        ret = TLSX_GetSize(ssl->extensions, semaphore, msgType, &length, &ssl->pqInfo);
        if (ret != 0)
            return ret;
    }
    if (ssl->ctx && ssl->ctx->extensions) {
        ret = TLSX_GetSize(ssl->ctx->extensions, semaphore, msgType, &length, &ssl->pqInfo);
        if (ret != 0)
            return ret;
    }
