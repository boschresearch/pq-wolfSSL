 ...
 * WOLFSSL_DEBUG_PQC
 *    Writes out details of Post-Quantum TLS 1.3 protocols: keys and other secrets
 * WOLFSSL_EARLY_DATA
...
int SendTls13ClientHello(WOLFSSL* ssl)
{
    ...
    /* PQ evaluation */
    ssl->pqInfo.clientHelloSz = sendSz;
...
int SendTls13ServerHello(WOLFSSL* ssl, byte extMsgType)
{
    ...
    /* PQ evaluation */
    ssl->pqInfo.serverHelloSz = sendSz;
                                       ...
                                       int reqCtxLen)
{
    ...
    if (ssl->options.side == WOLFSSL_SERVER_END)
        InitSuitesHashSigAlgo(ssl->suites, 1, 1, ssl->options.haveDilithiumSig,
                    ssl->options.haveFalconSig, ssl->options.haveSphincsSig,
                    ssl->options.haveXmssSig, 0, 1, ssl->buffers.keySz);
    ...
    /* PQ evaluation */
    ssl->pqInfo.certificateRequestSz = sendSz;
    ...
    defined(HAVE_ED448) || defined(HAVE_DILITHIUM) || defined(HAVE_FALCON) || \
...
#endif
#ifdef HAVE_DILITHIUM
        case dilithium_sa_algo:
            output[0] = hashAlgo;
            output[1] = dilithium_sa_algo;
            break;
#endif
#ifdef HAVE_FALCON
        case falcon_sa_algo:
            output[0] = hashAlgo;
            output[1] = falcon_sa_algo;
            break;
#endif
#ifdef HAVE_SPHINCS
        case sphincs_sa_algo:
            output[0] = hashAlgo;
            output[1] = sphincs_sa_algo;
            break;
#endif
#ifdef HAVE_XMSS
        case xmss_sa_algo:
            output[0] = hashAlgo;
            output[1] = xmss_sa_algo;
            break;
#endif
    }
}
...
static int SendTls13Certificate(WOLFSSL* ssl)
{
    ...
    while (length > 0 && ret == 0) {
        ...
        if (ssl->options.side == WOLFSSL_CLIENT_END)
            ssl->pqInfo.clientCertificateSz += sendSz;
        else if (ssl->options.side == WOLFSSL_SERVER_END)
            ssl->pqInfo.serverCertificateSz += sendSz;
    ...
    switch(ssl->options.asyncState)
    {
        ...
        case TLS_ASYNC_BUILD:
        {
            ...
            if (ssl->hsType == DYNAMIC_TYPE_RSA)
                ...
                args->sigAlgo = dilithium_sa_algo;
        #endif
        #ifdef HAVE_FALCON
            else if (ssl->hsType == DYNAMIC_TYPE_FALCON)
                args->sigAlgo = falcon_sa_algo;
        #endif
        #ifdef HAVE_SPHINCS
            else if (ssl->hsType == DYNAMIC_TYPE_SPHINCS)
                args->sigAlgo = sphincs_sa_algo;
        #endif
        #ifdef HAVE_XMSS
            else if (ssl->hsType == DYNAMIC_TYPE_XMSS)
                args->sigAlgo = xmss_sa_algo;
        #endif
            else {
            ...
            if (ssl->hsType == DYNAMIC_TYPE_DILITHIUM) {
                sig->length = DILITHIUM_SIG_SIZE;
            }
        #endif /* HAVE_DILITHIUM */
        #ifdef HAVE_FALCON
            if (ssl->hsType == DYNAMIC_TYPE_FALCON) {
                sig->length = FALCON_SIG_SIZE;
            }
        #endif /* HAVE_FALCON */
        #ifdef HAVE_SPHINCS
            if (ssl->hsType == DYNAMIC_TYPE_SPHINCS) {
                sig->length = SPHINCS_SIG_SIZE;
            }
        #endif /* HAVE_SPHINCS */
        #ifdef HAVE_XMSS
            if (ssl->hsType == DYNAMIC_TYPE_XMSS) {
                sig->length = XMSS_MAX_SIG_SIZE;
            }
        #endif /* HAVE_XMSS */
        ...
        #ifdef HAVE_DILITHIUM
             if (ssl->hsType == DYNAMIC_TYPE_DILITHIUM) {
                 wc_DilithiumSign (args->verify + HASH_SIG_SIZE + VERIFY_HEADER,
                    (size_t*)&sig->length, args->sigData, (size_t)args->sigDataSz,
                    (DilithiumKey*)ssl->hsKey, ssl->rng);
                args->length = (word16)sig->length;
            }
        #endif /* HAVE_DILITHIUM */
        #ifdef HAVE_FALCON
             if (ssl->hsType == DYNAMIC_TYPE_FALCON) {
                 wc_FalconSign (args->verify + HASH_SIG_SIZE + VERIFY_HEADER,
                    (size_t*)&sig->length, args->sigData, (size_t)args->sigDataSz,
                    (FalconKey*)ssl->hsKey, ssl->rng);
                args->length = (word16)sig->length;
            }
        #endif /* HAVE_FALCON */
        #ifdef HAVE_SPHINCS
             if (ssl->hsType == DYNAMIC_TYPE_SPHINCS) {
                size_t spx_outlen = sig->length + (size_t)args->sigDataSz;

                //SPX+ Signature too large -> requires fragmentation, use signature buffer as temporary storage
                if (spx_outlen > (size_t) wolfSSL_GetMaxRecordSize(ssl, MAX_RECORD_SIZE)) {

                    /* build encoded signature buffer */
                    sig->buffer = (byte*)XMALLOC(spx_outlen, ssl->heap, DYNAMIC_TYPE_SIGNATURE);
                    if (sig->buffer == NULL) {
                        ERROR_OUT(MEMORY_E, exit_scv);
                    }

                    wc_SphincsSign (sig->buffer,
                        (long long unsigned int*)&sig->length, args->sigData, (size_t)args->sigDataSz,
                        (SphincsKey*)ssl->hsKey, ssl->rng);

                    XMEMSET(args->verify + HASH_SIG_SIZE + VERIFY_HEADER, 0, sig->length*sizeof(byte)); //indicates fragmentation

                }
                else { //no fragmentation required

                    wc_SphincsSign (args->verify + HASH_SIG_SIZE + VERIFY_HEADER,
                    (long long unsigned int*)&sig->length, args->sigData, (size_t)args->sigDataSz,
                    (SphincsKey*)ssl->hsKey, ssl->rng);
        ...
        #ifdef HAVE_XMSS
             if (ssl->hsType == DYNAMIC_TYPE_XMSS) {
                 wc_XmssSign (args->verify + HASH_SIG_SIZE + VERIFY_HEADER,
                    (long long unsigned int*)&sig->length, args->sigData, (size_t)args->sigDataSz,
                    (XmssKey*)ssl->hsKey);
                args->length = (word16)sig->length;
            }
        ...
        case TLS_ASYNC_FINALIZE:
        {
            ...
            if(sig->buffer != NULL && (args->verify[HASH_SIG_SIZE + VERIFY_HEADER] == 0)) {
                word32 copySz;
                /* Put the record and handshake headers on. */
                word32 length = args->length + HASH_SIG_SIZE + VERIFY_HEADER;  // Sig Length + Hash Sig Size + Verify Header
                ...
                while (length > 0 && ret == 0)
                {
                    ...
                    if (ssl->fragOffset == 0) {
                        AddTls13FragHeaders(args->output, fragSz, 0, payloadSz, certificate_verify, ssl);
                        length -= HASH_SIG_SIZE + VERIFY_HEADER;
                        fragSz -= HASH_SIG_SIZE + VERIFY_HEADER;
                        i += HASH_SIG_SIZE + VERIFY_HEADER;
                    }
                    else {
                        AddTls13RecordHeader(args->output, fragSz, handshake, ssl);
                    }

                    /* Copy signature from buffer to output */
                    if (length > fragSz) {
                        copySz = min(length, fragSz);
                        XMEMCPY(args->output + i, sig->buffer + ssl->fragOffset, copySz);
                    }
                    else {
                        copySz = min(length - ssl->fragOffset, fragSz);
                        XMEMCPY(args->output + i, sig->buffer + ssl->fragOffset, copySz);
                    }
                    ...
                    /* This message is always encrypted. */
                    ret = BuildTls13Message(ssl, args->output, args->sendSz,
                    ...
                    //TODO: wolfSSL callbakcs missing -> requires further testing

                    /* PQ evaluation */
                    if (ssl->options.side == WOLFSSL_CLIENT_END)
                        ssl->pqInfo.clientCertificateVerifySz += args->sendSz;
                    else if (ssl->options.side == WOLFSSL_SERVER_END)
                        ssl->pqInfo.serverCertificateVerifySz += args->sendSz;
                                    ...
                                    word32* inOutIdx, word32 totalSz)
{
    ...
    switch(ssl->options.asyncState)
    {
        ...
        #endif
        #ifdef HAVE_DILITHIUM
            if (args->sigAlgo == dilithium_sa_algo &&
                                                   !ssl->peerDilithiumKeyPresent) {
                WOLFSSL_MSG("Peer sent Dilithium sig but not Dilithium cert");
                ret = SIG_VERIFY_E;
                goto exit_dcv;
            }
        #endif
        #ifdef HAVE_FALCON
            if (args->sigAlgo == falcon_sa_algo &&
                                                   !ssl->peerFalconKeyPresent) {
                WOLFSSL_MSG("Peer sent Falcon sig but not Falcon cert");
                ret = SIG_VERIFY_E;
                goto exit_dcv;
            }
        #endif
        #ifdef HAVE_SPHINCS
            if (args->sigAlgo == sphincs_sa_algo &&
                                                   !ssl->peerSphincsKeyPresent) {
                WOLFSSL_MSG("Peer sent Sphincs+ sig but not Sphincs+ cert");
                ret = SIG_VERIFY_E;
                goto exit_dcv;
            }
        #endif
        #ifdef HAVE_XMSS
            if (args->sigAlgo == xmss_sa_algo &&
                                                   !ssl->peerXmssKeyPresent) {
                WOLFSSL_MSG("Peer sent XMSS sig but not XMSS cert");
                ret = SIG_VERIFY_E;
                goto exit_dcv;
            }
        #endif
       ...
       #endif
       #ifdef HAVE_XMSS
            if (ssl->peerXmssKeyPresent) {
                WOLFSSL_MSG("Doing XMSS peer cert verify");
        ...
        case TLS_ASYNC_DO:
        {
            ...
            if (ssl->peerDilithiumKeyPresent) {
                WOLFSSL_MSG("Doing Dilithium peer cert verify");
                ret = wc_DilithiumVerify(args->sigData, (size_t*)&args->sigDataSz,
                                    input + args->idx, (size_t)args->sz,
            ...
            if (ssl->peerFalconKeyPresent) {
                WOLFSSL_MSG("Doing Falcon peer cert verify");
                ret = wc_FalconVerify(args->sigData, (size_t*)&args->sigDataSz,
                                    input + args->idx, (size_t)args->sz,
                                    ssl->peerFalconKey);
            ...
            if (ssl->peerSphincsKeyPresent) {
                WOLFSSL_MSG("Doing Sphincs peer cert verify");
                ret = wc_SphincsVerify(args->sigData, (long long unsigned int*)&args->sigDataSz,
                                    input + args->idx, (size_t)args->sz,
                                    ssl->peerSphincsKey);
            ...
            if (ssl->peerXmssKeyPresent) {
                WOLFSSL_MSG("Doing XMSS peer cert verify");
                ret = wc_XmssVerify(args->sigData, (long long unsigned int*)&args->sigDataSz,
                                    input + args->idx, (size_t)args->sz,
                                    ssl->peerXmssKey);

                if (ret >= 0) {
                    FreeKey(ssl, DYNAMIC_TYPE_XMSS,
                                                    (void**)&ssl->peerXmssKey);
                    ssl->peerXmssKeyPresent = 0;
                }
            }
        #endif
...
#if !defined(NO_RSA) || defined(HAVE_ECC) || defined(HAVE_ED25519) || \
                defined(HAVE_ED448) || defined(HAVE_DILITHIUM) || \
                        ...
                        word32 totalSz)
{
    ...
    if (ssl->arrays == NULL) {
        ...
        if (GetHandshakeHeader(ssl, input, inOutIdx, &type, &size,
                                                                totalSz) != 0) {
            WOLFSSL_MSG("DoTls13HandShakeMsg() FAILED.");
            SendAlert(ssl, alert_fatal, unexpected_message);
            return PARSE_ERROR;
        }
                                           ...
                                           wc_psk_client_tls13_callback cb)
{
    ...
    InitSuites(ssl->suites, ssl->version, keySz, haveRSA, TRUE,
               ...
               ssl->options.haveStaticECC, ssl->options.haveDilithiumSig,
               ssl->options.haveFalconSig, ssl->options.haveSphincsSig,
               ssl->options.haveXmssSig, ssl->options.side);
}
